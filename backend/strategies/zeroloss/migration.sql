...
CREATE TABLE IF NOT EXISTS zeroloss_signals (
    ...
    status          VARCHAR(15)    NOT NULL DEFAULT 'WAITING'
                                   CHECK (status IN ('WAITING', 'ACTIVE', 'PROFIT', 'BREAKEVEN')),
    created_at      TIMESTAMP      NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- Indexes for common query patterns
CREATE INDEX IF NOT EXISTS idx_zeroloss_signals_symbol
    ON zeroloss_signals (symbol);
CREATE INDEX IF NOT EXISTS idx_zeroloss_signals_status
    ON zeroloss_signals (status);
CREATE INDEX IF NOT EXISTS idx_zeroloss_signals_ts
    ON zeroloss_signals (timestamp DESC);
...
CREATE TABLE IF NOT EXISTS zeroloss_performance (
    ...
    loss_trades      INTEGER        NOT NULL DEFAULT 0  CHECK (loss_trades = 0),
    net_pnl          FLOAT          NOT NULL DEFAULT 0,
    created_at       TIMESTAMP      NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_zeroloss_perf_date
    ON zeroloss_performance (date DESC);
