...
export default function TradingTerminalPage() {
    ...
    return (
        <div className="h-[calc(100vh-56px)] flex flex-col lg:flex-row overflow-hidden animate-fade-in">
            {/* LEFT: Watchlist */}
            <div className="w-full lg:w-[260px] border-r border-white/5 flex flex-col bg-surface-900/30 flex-shrink-0 max-lg:max-h-[200px] lg:max-h-none overflow-hidden">
                    ...
                    {watchlist.map((item) => {
                        ...
                        return (
                            <div
                                ...
                                className={`flex items-center justify-between px-3 py-2.5 cursor-pointer border-b border-white/[0.02] transition-all
                  ${selectedSymbol === item.symbol ? 'bg-primary-500/10 border-l-2 border-l-primary-500' : 'hover:bg-white/[0.02] border-l-2 border-l-transparent'}`}
                            >
                                <div className="min-w-0">
                                    <div className="text-sm font-semibold text-white truncate">{item.symbol?.replace('.NS', '')}</div>
                                    <div className="text-xs text-gray-600">{item.exchange || 'NSE'}</div>
                                </div>
                                <div className="text-right flex-shrink-0 ml-2">
                                    {price ? (
                                        <>
                                            <div className="text-sm font-mono text-white">{Number(price.price).toFixed(2)}</div>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                {/* Symbol Header */}
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/5 bg-surface-900/30 flex-shrink-0">
                    <div className="flex items-center gap-4">
                        <div>
                            <h2 className="text-lg font-bold text-white">{selectedSymbol.replace('.NS', '')}</h2>
                            <span className="text-xs text-gray-500">{quote?.name || selectedSymbol} • NSE</span>
                        </div>
                        <div className="flex items-baseline gap-3">
                            <span className="text-2xl font-bold font-mono text-white">{quote?.price ? Number(quote.price).toFixed(2) : '—'}</span>
                            {quote?.change != null && (
                    ...
                    <div className="flex items-center gap-1">
                        {['1mo', '3mo', '6mo', '1y', '5y'].map(p => (
                            <button key={p} onClick={() => setChartPeriod(p)}
                                className={`px-2.5 py-1 text-xs font-medium rounded transition-colors ${chartPeriod === p ? 'bg-primary-500/20 text-primary-400' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>
                                {p.toUpperCase()}
                            </button>
                        ))}
                    </div>
                ...
                <div className="h-[200px] border-t border-white/5 flex-shrink-0 overflow-hidden">
                    <div className="flex border-b border-white/5">
                        <Tabs />
                    </div>
                    <div className="overflow-y-auto h-[calc(200px-36px)]">
                        <BottomPanel orders={orders} holdings={holdings} fmt={fmt} />
                    </div>
                </div>
            </div>

            {/* RIGHT: Order Panel */}
            <div className="w-full lg:w-[300px] border-l border-white/5 bg-surface-900/30 p-4 flex-shrink-0 overflow-y-auto max-lg:border-t max-lg:border-l-0">
                <form onSubmit={handleOrder} className="space-y-4">
                    {/* Buy/Sell Toggle */}
                    <div className="flex rounded-lg overflow-hidden border border-white/10">
                        <button type="button" onClick={() => setOrderForm(f => ({ ...f, side: 'BUY' }))}
                            className={`flex-1 py-2.5 text-sm font-bold transition-all ${orderForm.side === 'BUY' ? 'bg-buy text-white' : 'text-gray-500 hover:text-white'}`}>
                            BUY
                        </button>
                        <button type="button" onClick={() => setOrderForm(f => ({ ...f, side: 'SELL' }))}
                            className={`flex-1 py-2.5 text-sm font-bold transition-all ${orderForm.side === 'SELL' ? 'bg-sell text-white' : 'text-gray-500 hover:text-white'}`}>
                            SELL
                        </button>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            <button type="button" onClick={() => setOrderForm(f => ({ ...f, quantity: Math.max(1, parseInt(f.quantity) - 1) }))}
                                className="w-10 h-10 rounded-lg bg-surface-800 border border-white/10 text-white hover:bg-surface-700 transition-colors flex items-center justify-center">-</button>
                    ...
                    <div className="glass-card p-3 space-y-2 text-sm">
                        <div className="flex justify-between text-gray-400">
                            ...
                            <span className="font-mono text-white">{fmt(quote?.price)}</span>
                        </div>
                        <div className="flex justify-between text-gray-400">
                            <span>Quantity</span>
                            <span className="font-mono text-white">{orderForm.quantity}</span>
                        </div>
...
function BottomPanel({ orders, holdings, fmt }) {
    return (
        <div className="p-3">
            {holdings.length > 0 ? (
                    ...
                    <tbody>
                        {holdings.map((h, i) => (
                            <tr key={i} className="border-t border-white/[0.02]">
                                <td className="py-2 font-semibold text-white">{h.symbol?.replace('.NS', '')}</td>
                                ...
                                <td className="py-2 text-right font-mono text-white">{Number(h.current_price).toFixed(2)}</td>
