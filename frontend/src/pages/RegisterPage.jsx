...
import {
    ...
    HiOutlineEye, HiOutlineEyeOff, HiArrowRight,
    HiShieldCheck, HiCheckCircle, HiCurrencyRupee,
    HiChartBar, HiLightningBolt, HiTrendingUp,
} from 'react-icons/hi';
    ...
    return (
        <div className="mt-2 space-y-1">
            <div className="flex gap-1">
                {[1,2,3,4].map(i => (
                    <div key={i} className={`h-[3px] flex-1 rounded-full transition-all duration-300 ${i <= score ? cfg.bar : 'bg-white/8'}`} />
                ))}
            </div>
            <p className={`text-[10px] font-bold ${cfg.text}`}>{cfg.label} password</p>
        </div>
    );
}
...
function RegisterAnimation() {
    const portfolioItems = [
        { sym: 'RELIANCE', qty: 10, avg: 1380, ltp: 1420, pnl: '+₹400', up: true,  pct: '+2.9%' },
        { sym: 'TCS',      qty:  5, avg: 3450, ltp: 3598, pnl: '+₹740', up: true,  pct: '+4.3%' },
        { sym: 'HDFCBANK', qty: 20, avg: 1950, ltp: 1920, pnl: '-₹600', up: false, pct: '-1.5%' },
        { sym: 'INFY',     qty: 15, avg: 1310, ltp: 1281, pnl: '-₹435', up: false, pct: '-2.2%' },
        { sym: 'BAJFIN',   qty:  3, avg: 7600, ltp: 7845, pnl: '+₹735', up: true,  pct: '+3.2%' },
    ];
    ...
    const donutSlices = donutData.map(d => {
        ...
        const gap = 3;
        const slice = { ...d, dasharray: `${len - gap} ${circ - len + gap}`, dashoffset: -offset };
        offset += len;
        return slice;
    });
    ...
    return (
        <div className="relative w-full h-full flex flex-col overflow-hidden select-none">
            {/* Glows */}
            <div className="absolute -top-20 -left-20 w-72 h-72 rounded-full bg-indigo-600/12 blur-3xl" style={{ animation: 'pulse 4s ease-in-out infinite' }} />
            <div className="absolute bottom-20 right-0 w-64 h-64 rounded-full bg-violet-500/8 blur-3xl" style={{ animation: 'pulse 5s ease-in-out infinite' }} />

            {/* Logo */}
            <div className="relative z-10 px-9 pt-9 flex-shrink-0">
                <Link to="/"><img src="/logo.png" alt="AlphaSync" className="h-11 object-contain dark:brightness-100 brightness-0" /></Link>
            </div>
            ...
            <div className="relative z-10 flex-1 flex flex-col justify-center px-7 py-3 gap-3 overflow-hidden">
                ...
                <div className="rounded-2xl border border-white/8 overflow-hidden shadow-2xl shadow-black/60"
                    ...
                    {/* Header */}
                    <div className="flex items-center justify-between px-5 py-3 border-b border-white/5" style={{ background: 'rgba(0,0,0,0.3)' }}>
                        <span className="text-[11px] font-bold text-gray-400 tracking-widest uppercase">Portfolio</span>
                    ...
                    <div className="flex items-center justify-between px-5 py-3 border-b border-white/5">
                        <div>
                            <div className="text-[10px] text-gray-600 font-semibold uppercase tracking-wider">Total Value</div>
                            <div className="text-lg font-black text-white font-mono tracking-tight mt-0.5">₹10,02,840</div>
                        </div>
                        <div className="text-right">
                            <div className="text-[10px] text-gray-600 font-semibold uppercase tracking-wider">Day P&amp;L</div>
                            <div className="text-base font-black text-emerald-400 font-mono mt-0.5">+₹2,840</div>
                            <div className="text-[10px] text-emerald-500 font-mono">+0.28%</div>
                        </div>
                        {/* Mini donut */}
                            ...
                            <svg viewBox="0 0 88 88" className="w-full h-full -rotate-90">
                                {donutSlices.map((s, i) => (
                                    <circle key={i} cx="44" cy="44" r={r}
                                        fill="none" stroke={s.col} strokeWidth="10"
                    ...
                    <div className="divide-y divide-white/[0.04]">
                        {portfolioItems.map((item, i) => (
                            <div key={item.sym} className="flex items-center px-5 py-2.5 hover:bg-white/[0.02] transition-colors"
                                style={{ animation: `fadeInUp 0.3s ease-out ${i * 0.07}s both` }}>
                                <div className="w-16">
                                    <div className="text-[11px] font-black text-gray-300 tracking-wide">{item.sym}</div>
                                    <div className="text-[9px] text-gray-600 mt-0.5">{item.qty} shares</div>
                                </div>
                                <div className="flex-1 text-center">
                                    <div className="text-[11px] font-mono text-gray-400">₹{item.ltp.toLocaleString()}</div>
                                </div>
                                <div className="text-right">
                                    <div className={`text-[11px] font-black font-mono ${item.up ? 'text-emerald-400' : 'text-red-400'}`}>{item.pnl}</div>
                                    <div className={`text-[9px] font-mono ${item.up ? 'text-emerald-500' : 'text-red-500'}`}>{item.pct}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex gap-2 flex-wrap">
                    {donutData.map(d => (
                        <div key={d.label} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-white/8 bg-white/[0.03]">
                            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: d.col }} />
                            <span className="text-[10px] text-gray-500 font-semibold">{d.label} {d.pct}%</span>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="relative z-10 px-9 pb-8 flex-shrink-0">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-indigo-500/20 bg-indigo-500/8 mb-3">
                    ...
                    <span className="text-[11px] font-semibold text-indigo-300 tracking-wide">Join 500+ active traders today</span>
                </div>
                ...
                <p className="text-[12px] text-gray-600 mt-1.5">Real data. Virtual money. Zero risk.</p>
            </div>
...
export default function RegisterPage() {
    ...
    const fieldCls = "w-full py-2.5 rounded-xl text-sm text-white placeholder-gray-600 border transition-all duration-200 focus:outline-none focus:ring-1 focus:ring-indigo-500/25 focus:border-indigo-500/50 focus:bg-white/[0.06]";
    const fieldBg  = "bg-white/[0.04] border-white/8";
    const passwordsMatch = formData.confirmPassword && formData.password === formData.confirmPassword;

    return (
        <div className="h-screen w-screen overflow-hidden flex" style={{ background: '#07080f' }}>

            {/* LEFT — animated portfolio panel */}
            <div className="hidden lg:flex w-[50%] h-full border-r border-white/5 flex-col"
                style={{ background: 'linear-gradient(135deg, #0a0c1a 0%, #0d0f20 50%, #08090f 100%)' }}>
                <RegisterAnimation />
            </div>

            {/* RIGHT — premium form */}
            <div className="flex-1 h-full flex items-center justify-center px-7 xl:px-10 relative overflow-hidden"
                style={{ background: 'linear-gradient(145deg, #0b0d1c 0%, #07080f 100%)' }}>
                ...
                <div className="lg:hidden absolute top-7 left-1/2 -translate-x-1/2">
                    <Link to="/"><img src="/logo.png" alt="AlphaSync" className="h-10 object-contain" /></Link>
                </div>
                ...
                <div className="w-full max-w-[400px] relative z-10">
                    ...
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-indigo-500/20 bg-indigo-500/8 mb-5">
                        ...
                        <span className="text-[11px] font-semibold text-indigo-300 tracking-wide">Free · No KYC · No Bank Details</span>
                    </div>
                    ...
                    <div className="relative">
                        <div className="absolute -inset-[1px] rounded-2xl pointer-events-none"
                            style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.22) 0%, rgba(255,255,255,0.03) 50%, rgba(167,139,250,0.12) 100%)' }} />
                        <div className="relative rounded-2xl p-5 shadow-2xl"
                            style={{ background: 'rgba(14,16,30,0.96)', backdropFilter: 'blur(24px)' }}>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-3.5">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative group">
                                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[12px] text-gray-600 font-mono group-focus-within:text-indigo-400 transition-colors select-none">@</span>
                                ...
                                {/* Confirm Password */}
                                <div>
                                    <label className="block text-[10px] font-bold text-gray-500 mb-1.5 uppercase tracking-widest">Confirm Password</label>
                                    <div className="relative group">
                                        <HiOutlineLockClosed className="absolute left-3 top-1/2 -translate-y-1/2 w-[14px] h-[14px] text-gray-600 group-focus-within:text-indigo-400 transition-colors" />
                                        ...
                                        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                                            {passwordsMatch && <HiCheckCircle className="w-4 h-4 text-emerald-400" />}
                                    ...
                                    {formData.confirmPassword && !passwordsMatch && (
                                        <p className="text-[10px] text-red-400 font-bold mt-1">Passwords do not match</p>
                                    )}
                                </div>
                                ...
                                <div className="pt-1">
                                    <button type="submit" disabled={loading}
                                        ...
                                        style={{ background: 'linear-gradient(135deg, #6366f1 0%, #4f46e5 60%, #4338ca 100%)', boxShadow: '0 4px 24px rgba(99,102,241,0.35)' }}>
                                        ...
                                        {loading ? (
                                            ...
                                            <>Create Free Account <HiArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform duration-150" /></>
                                        )}
                                    </button>
                                </div>
                            </form>
                    ...
                    <div className="mt-4 text-center space-y-2">
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                            ...
                            <span className="text-[11px] text-gray-700">Free forever. No credit card required.</span>
                        </div>
                    </div>
                </div>
            </div>
