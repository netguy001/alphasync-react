...
import {
    HiArrowRight, HiChartBar, HiLightningBolt, HiShieldCheck,
    HiCurrencyRupee, HiTrendingUp, HiCog, HiCheckCircle,
} from 'react-icons/hi';
...
const FEATURES = [
    { icon: HiChartBar, title: 'Professional Charts', desc: 'TradingView-powered candlestick charts with 20+ technical indicators and multi-timeframe analysis.' },
    { icon: HiLightningBolt, title: 'Instant Order Execution', desc: 'Market, limit, and stop-loss orders with real-time fills and live position tracking.' },
    { icon: HiCog, title: 'Algo Strategy Engine', desc: 'Build and deploy SMA, RSI, MACD and Bollinger Band strategies. Automated execution.' },
    { icon: HiShieldCheck, title: 'Risk Management', desc: 'Position sizing, stop-loss rules, drawdown limits, and portfolio-level risk analytics.' },
    { icon: HiCurrencyRupee, title: '₹10,00,000 Virtual Capital', desc: 'Start with ₹10L in simulated funds. No real money, zero risk, full learning.' },
    { icon: HiTrendingUp, title: 'Portfolio Analytics', desc: 'Real-time P&L, win rate analysis, sector allocation, and performance benchmarking.' },
];

const STEPS = [
    { n: '01', title: 'Create Free Account', desc: 'Sign up in 30 seconds. No KYC, no bank details, no real money ever required.' },
    { n: '02', title: 'Get ₹10L Capital', desc: 'Your account is instantly loaded with ₹10,00,000 in virtual funds to trade with.' },
    { n: '03', title: 'Start Trading', desc: 'Access real NSE/BSE data, place orders, run algo strategies, and master the markets.' },
];
...
/* ─── SVG Chart ──────────────────────────────────────────────── */
function AreaChart() {
    const line = "0,70 40,62 80,75 120,50 160,56 200,34 240,48 280,24 320,38 360,16 400,30 440,8 480,22 520,0";
    return (
        <svg viewBox="0 0 520 80" className="w-full h-16" preserveAspectRatio="none">
            <defs>
            ...
            <polyline points={line} fill="none" stroke="#6366f1" strokeWidth="2" strokeLinejoin="round" />
        </svg>
    );
}
    ...
    return (
        <div className="overflow-hidden border-y border-edge/5 bg-surface-900/50 py-2.5">
            <div className="flex animate-marquee will-change-transform">
                {items.map((item, i) => (
                    <div key={i} className="flex items-center gap-2.5 px-5 border-r border-edge/5 flex-shrink-0">
                        <span className="text-[11px] font-semibold text-gray-500 tracking-wide uppercase">{item.s}</span>
                        <span className="text-[12px] font-mono text-gray-300">{item.p}</span>
                        <span className={`text-[11px] font-mono font-semibold ${item.up ? 'text-profit' : 'text-loss'}`}>{item.c}</span>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="min-h-screen bg-surface-950 text-heading">
            ...
            <nav className="sticky top-0 z-50 flex items-center justify-between px-6 lg:px-14 h-16 border-b border-edge/5 backdrop-blur-xl bg-surface-950/80">
                <div className="flex items-center gap-2.5">
                    <img src="/logo.png" alt="AlphaSync" className="h-14 rounded-lg object-contain dark:brightness-100 brightness-0" />
                </div>
                <div className="flex items-center gap-3">
                    {isAuthenticated ? (
                        ...
                        <>
                            <Link to="/login" className="text-gray-400 hover:text-heading text-sm font-medium transition-colors px-3 py-2">Sign In</Link>
                ...
                <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
                    <div>
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-primary-500/20 bg-primary-500/8 mb-6">
                            ...
                            <span className="text-xs font-medium text-primary-300 tracking-wide">Live NSE &amp; BSE Market Data</span>
                        </div>
                        ...
                        <p className="text-lg text-gray-400 leading-relaxed mb-8 max-w-lg">
                            Practice with real NSE &amp; BSE data. Execute trades, run algo strategies, and build your
                            edge — with ₹10,00,000 in virtual capital. Risk nothing. Learn everything.
                        </p>
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 mb-10">
                            ...
                            <Link to="/login" className="btn-secondary inline-flex items-center justify-center gap-2 px-7 py-3.5 text-base">
                                Sign In to Account
                            </Link>
                        </div>
                    ...
                    <div className="hidden lg:block">
                        <div className="glass-card overflow-hidden shadow-panel">
                            <div className="flex items-center justify-between px-4 py-3 border-b border-edge/5 bg-surface-900/60">
                                ...
                                <span className="text-[11px] font-mono text-gray-500">RELIANCE.NS — 3M</span>
                            ...
                            <div className="flex items-center gap-5 px-4 py-2.5 bg-surface-900/40 border-b border-edge/5">
                                <div>
                                    <div className="text-xl font-bold text-heading font-mono">₹1,420.70</div>
                                    <div className="text-xs text-loss font-mono">▼ 7.30 (−0.51%)</div>
                                </div>
                                <div className="ml-auto flex items-center gap-1">
                                    {['1D', '1W', '1M', '3M', '1Y'].map(p => (
                                        <span key={p} className={`text-[11px] px-2 py-0.5 rounded font-medium ${p === '3M' ? 'bg-primary-500/20 text-primary-400' : 'text-gray-600'}`}>{p}</span>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="flex bg-surface-900/60 border-t border-edge/5 divide-x divide-edge/5">
                                {[
                                    ...
                                    <div key={s} className="flex-1 px-3 py-2.5 flex flex-col gap-0.5">
                                        <span className="text-[10px] font-semibold text-gray-500">{s}</span>
                                        <span className="text-xs font-mono text-gray-300">{p}</span>
                                        <span className={`text-[11px] font-mono font-medium ${up ? 'text-profit' : 'text-loss'}`}>{c}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="max-w-6xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-8">
                    {STATS.map(({ v, l }) => (
                        <div key={l} className="text-center">
                            <div className="text-3xl font-extrabold text-heading tracking-tight mb-1">{v}</div>
            ...
            <section className="px-6 lg:px-14 py-24">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-14">
                        <h2 className="text-3xl lg:text-4xl font-extrabold tracking-tight mb-3">Up and trading in minutes.</h2>
                        <p className="text-gray-400 text-lg">Three steps. No paperwork. No deposit.</p>
                    </div>
                            ...
                            <div key={i} className="relative">
                                ...
                                <div className="flex items-center gap-4 mb-4">
                                    <div className="w-14 h-14 rounded-xl bg-primary-600/15 border border-primary-500/15 flex items-center justify-center flex-shrink-0">
                                        <span className="text-lg font-black text-primary-400 font-mono">{n}</span>
                                    </div>
            ...
            <section className="px-6 lg:px-14 py-20 border-t border-edge/5">
                <div className="max-w-3xl mx-auto text-center">
                    <h2 className="text-3xl lg:text-5xl font-extrabold tracking-tight mb-5">
                        Ready to trade
                        <span className="block bg-gradient-to-r from-primary-400 to-accent-cyan bg-clip-text text-transparent">like a professional?</span>
                    </h2>
                    <p className="text-gray-400 text-lg mb-8 max-w-xl mx-auto">
                        Join 500+ traders already practising on AlphaSync. Create your free account and start trading today.
                    </p>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <Link to="/register" className="btn-primary inline-flex items-center justify-center gap-2 px-10 py-4 text-base">
                            Create Free Account <HiArrowRight className="w-5 h-5" />
                        </Link>
                        <Link to="/login" className="btn-secondary inline-flex items-center justify-center gap-2 px-10 py-4 text-base">
                            Sign In
                        </Link>
                    </div>
            ...
            <footer className="border-t border-edge/5 px-6 lg:px-14 py-8">
                <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-2.5">
                        <img src="/logo.png" alt="AlphaSync" className="h-10 rounded-lg object-contain dark:brightness-100 brightness-0" />
                        <p className="text-xs text-gray-600">Simulation Trading Platform</p>
                    </div>
                    <div className="flex items-center gap-6 text-xs text-gray-500">
                        {['Privacy Policy', 'Terms of Service', 'Support', 'GitHub'].map(l => (
