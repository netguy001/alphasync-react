...
export function runEngine(candles, enabledIds = []) {
    ...
    if (activeStrategies >= 2) {
        ...
        if (normScore > 0.25 && bullish > bearish) {
            overall = 'BULLISH';
            // Confidence = weighted average of bullish strategies' confidence
            confidence = bullWeightSum > 0 ? Math.round(bullConfSum / bullWeightSum) : 0;
            // Penalise if too many strategies are neutral (uncertain market)
            const activeFraction = activeStrategies / totalStrategies;
            if (activeFraction < 0.6) confidence = Math.round(confidence * 0.8);
        } else if (normScore < -0.25 && bearish > bullish) {
            overall = 'BEARISH';
