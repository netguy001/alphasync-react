...
export function runStrategy(candles) {
    ...
    const aboveSma = smaDeviation > 0.5;
    const belowSma = smaDeviation < -0.5;
    ...
    const rsiInDeadZone = currentRsi >= 45 && currentRsi <= 55;
    ...
    if (aboveSma && !rsiInDeadZone && currentRsi > 55 && rsiRising && emaSlopeUp) {
        signal = 'Bullish';
    ...
    else if (belowSma && !rsiInDeadZone && currentRsi < 45 && rsiFalling && emaSlopeDown) {
        signal = 'Bearish';
        confirms.push('SMA50▼', 'RSI▼', 'EMA9 slope▼');
        confidence = currentRsi < 40 ? 82 : 70;
    }
    // Partial bullish — 2 of 3 conditions
    else if (aboveSma && currentRsi > 55 && (rsiRising || emaSlopeUp)) {
        signal = 'Bullish';
        confidence = 58;
        confirms.push('Partial confluence (2/3)');
    }
    // Partial bearish — 2 of 3 conditions
    else if (belowSma && currentRsi < 45 && (rsiFalling || emaSlopeDown)) {
        signal = 'Bearish';
        confidence = 58;
        confirms.push('Partial confluence (2/3)');
    }
